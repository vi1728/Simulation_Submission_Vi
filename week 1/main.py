...
while running:
    ...
    for i in range(len(positions)):
        velocities[i] = velocities[i] + np.array([0.0, GRAVITY * dt])
    
        positions[i] += velocities[i] * dt

    offset = positions[i] - BOWL_CENTER #vector
    distance=np.linalg.norm(offset) #scalar
    normal=offset/distance #vector
    limit=BOWL_RADIUS-PARTICLE_RADIUS
    if distance>limit:
        normal_vel=np.dot(velocities[i],normal)
        if normal_vel>0:
            velocities[i]= -(1+WALL_RESTITUTION)*normal_vel*normal
